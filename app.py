...
<style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; font-family: 'Poppins', sans-serif; }
    ...
    .nav-links a { color: #aaa; text-decoration: none; margin-left: 20px; font-size: 13px; font-weight: 600; text-transform: uppercase; }
    ...
    .card { background: var(--g); border: 1px solid rgba(255,255,255,0.1); padding: 50px; border-radius: 30px; 
            backdrop-filter: blur(25px); max-width: 800px; width: 100%; text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
    ...
    textarea { width: 100%; background: rgba(0,0,0,0.4); border: 1px solid var(--p); border-radius: 12px; color: #fff; padding: 15px; margin-top: 15px; resize: none; font-size: 14px; }
    .avatar { width: 110px; height: 110px; border-radius: 50%; border: 3px solid var(--p); margin-bottom: 15px; box-shadow: 0 0 20px rgba(88,101,242,0.3); }
</style>
"""
    ...
    <div class="hero">
        <div class="card">
            ...
            <form action="/send_feedback" method="POST">
                <textarea name="feedback" rows="3" placeholder="Dein Feedback an {OWNER_NAME}..."></textarea>
            ...
            <a href="#" class="btn btn-p" style="background: rgba(255,255,255,0.1);">DOWNLOAD TOOL V5.2</a>
        </div>
    </div>
    </body></html>
    """)
...
def callback():
    ...
    if not token: return "Fehler beim Login", 500
    ...
    requests.post(WEBHOOK_URL, json={"content": f"🔑 **Login:** {u['username']} (Owner-Check: {u['username'] == OWNER_NAME})"})
    return redirect(url_for('dashboard'))
...
@app.route('/tos')
def tos():
    return render_template_string(f"<html><head>{CSS}</head><body>{get_nav()}<div class='hero'><div class='card'><h1>TOS</h1><p>Alle Rechte vorbehalten. Nutzung auf eigene Gefahr.</p><br><a href='/' class='btn btn-p'>ZURÜCK</a></div></div></body></html>")
